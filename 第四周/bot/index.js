...
async function startIndex() {
    let config;
    try {
        const configPath = path.resolve('config.json');  // 使用 path.resolve 指向當前工作目錄中的 config.json
    ...
    const eventPath = path.resolve('events');  // 使用 path.resolve 確保事件文件夾路徑正確
...
async function loadCommands(client) {
    const commandFiles = await getFiles(path.resolve('commands'));  // 使用 path.resolve 確保路徑正確
